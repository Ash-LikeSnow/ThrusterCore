    ...
    public partial class Session
    {
        ...
        internal void LoadDefinitions()
        {
            foreach (var def in MyDefinitionManager.Static?.GetAllDefinitions())
            {
                ...
                if (thrustDef != null)
                    _vanillaDefs.Add(thrustDef);
            }
        }

        internal void LoadThrusterCoreDefs()
        {
        ...
        private void ImportDefinitions(Definition[] definitions)
        {
            ...
            for (int i = 0; i < definitions.Length; i++)
            {
                ...
                if (definition.ThrustValues == null) continue;

                if (definition.Id.TypeId == _sorterType)
                {
                    var thrustDef = new ThrustDefinition(definition.ThrustValues);
                    DefinitionMap[definition.Id] = thrustDef;
                }
